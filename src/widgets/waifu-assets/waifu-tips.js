...
const  initModel = function initModel(waifuPath, type) {
  ...
  var waifuResize = window.waifuResize = function() { $(window).width() <= Number(live2d_settings.waifuMinWidth.replace('px','')) ? $(".waifu").hide() : $(".waifu").show(); };
...
function xssParser(text){
  return text.replace(/<img/ig, '&lt;img')
    .replace(/<sc/ig, '&lt;')
    .replace(/\\/g, '\\\\')
    .replace()
}
...
function loadTipsMessage(result) {
  ...
  $.each(result.click, function (index, tips){
    $(document).on("click", tips.selector, function (){
      // 这里嵌入干预代码
      var text =  xssParser(window.getHelpText(tips.selector) || getRandText(tips.text));
      text = text.render({text: $(this).text()});
  ...
  var showWelcomeMessage = window.showWelcomeMessage = function(result) {
    var text;
  ...
  if (live2d_settings.showHitokoto) {
    var getActed = window.getActed = false; var hitokotoTimer = window.hitokotoTimer = 0; var hitokotoInterval = window.hitokotoInterval = false;
